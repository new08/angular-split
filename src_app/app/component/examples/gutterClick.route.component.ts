      ...
      <div class="container">
          ...
          <div class="split-example">
              <as-split #mySplit
                        ...
                        (gutterLeftClick)="log('gutterLeftClick', $event)"
                        (gutterRightClick)="log('gutterRightClick', $event)">
          ...
          <div class="btns">
              <div>
                  <button class="btn btn-warning" [class.active]="!useTransition"
                          (click)="useTransition = !useTransition">{{ 'useTransition: ' + useTransition }}</button>
              </div>
              <div>
                  <button class="btn btn-warning" [class.active]="!isDisabled"
                          (click)="isDisabled = !isDisabled">{{ 'isDisabled: ' + isDisabled }}</button>
              </div>
              <div>
...
export class GutterClickComponent extends AComponent implements AfterViewInit, OnDestroy {
  ...
  log(type: string, e: { gutterNum: number, sizes: Array<number> }) {
    ...
    if (type === 'gutterClick') {
      this.gutterClick(e);
    } else if (type === 'gutterLeftClick') {
      this.gutterLeftClick(e);
    } else if (type === 'gutterRightClick') {
      this.gutterRightClick(e);
  ...
  gutterLeftClick(e: { gutterNum: number, sizes: Array<number> }) {
    if (e.gutterNum === 1) {
      const deltaSize = (this.areas[0].size > 0) ? this.areas[0].size : 0;
      this.areas[1].size += deltaSize;
      this.areas[0].size -= deltaSize;
    } else if (e.gutterNum === 2) {
      const deltaSize = (this.areas[1].size > 25 && this.areas[2].size === 0) ? 25 : this.areas[1].size;
      this.areas[2].size += deltaSize;
      this.areas[1].size -= deltaSize;
    }
  }

  gutterRightClick(e: { gutterNum: number, sizes: Array<number> }) {
    if (e.gutterNum === 1) {
      const deltaSize = (this.areas[1].size > 25 && this.areas[0].size === 0) ? 25 : this.areas[1].size;
      this.areas[0].size += deltaSize;
      this.areas[1].size -= deltaSize;
    } else if (e.gutterNum === 2) {
      const deltaSize = (this.areas[2].size > 0) ? this.areas[2].size : 0;
      this.areas[1].size += deltaSize;
      this.areas[2].size -= deltaSize;
    }
  }
