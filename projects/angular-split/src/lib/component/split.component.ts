    ...
    template: `
        ...
        <ng-template ngFor [ngForOf]="displayedAreas" let-index="index" let-last="last">
            <div
                ...
                <div class="as-split-gutter-panel">
                  <div class="as-split-gutter-button-left" title="<" (mousedown)="$event.stopPropagation()" (mouseup)="areaCollapse($event, index*2+1, index+1, 'left')">
                  ...
                  <div class="as-split-gutter-button-right" title=">" (mousedown)="$event.stopPropagation()" (mouseup)="areaCollapse($event, index*2+1, index+1, 'right')">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M5.88 4.12L13.76 12l-7.88 7.88L8 22l10-10L8 2z"/><path fill="none" d="M0 0h24v24H0z"/></svg>                  </div>
                </div>
            </div>
        </ng-template>`,
})
export class SplitComponent implements AfterViewInit, OnDestroy {
    ...
    @Output() gutterLeftClick = new EventEmitter<IOutputData>(false);
    @Output() gutterRightClick = new EventEmitter<IOutputData>(false);
    ...
    private refreshStyleSizes(): void {
        ...
        if (this.unit === 'percent') {
            ...
            if (this.displayedAreas.length === 1) {
                this.displayedAreas[0].component.setStyleFlex(0, 0, `calc( 100% -12px )`, false, false);
            } else {
            ...
            this.displayedAreas.forEach(area => {
                ...
                if (area.size === null) {
                    if (this.displayedAreas.length === 1) {
                        area.component.setStyleFlex(1, 1, `calc( 100% -12px )`, false, false);
                    } else {
                    ...
                    // Only one area > flex-basis 100%
                    if (this.displayedAreas.length === 1) {
                        area.component.setStyleFlex(0, 0, `calc( 100% -12px )`, false, false);
                    } else {
                        area.component.setStyleFlex(
                            0, 0, `${ area.size }px`,
                            (area.minSize !== null && area.minSize === area.size) ? true : false,
    ...
    public areaCollapse(event: MouseEvent | TouchEvent, gutterOrder: number, gutterNum: number, direction: 'left' | 'right'): void {
        event.preventDefault();
        event.stopPropagation();

        // const areas2Hide = direction === 'left'
        //   ? this.displayedAreas.filter(area => area.order < gutterOrder)
        //   : this.displayedAreas.filter(area => area.order > gutterOrder);
        // const areas2Display = direction === 'right'
        //   ? this.hidedAreas.filter(area => area.order < gutterOrder)
        //   : this.hidedAreas.filter(area => area.order > gutterOrder);
        // areas2Hide.forEach(area => {
        //     this.hideArea(area.component);
        // });
        // areas2Display.forEach(area => {
        //     this.showArea(area.component);
        // });

        this.notify(direction, gutterNum);
    }
    ...
    public notify(type: 'start' | 'progress' | 'end' | 'click' | 'dblclick' | 'transitionEnd' | 'left' | 'right', gutterNum: number): void {
        ...
        if (type === 'start') {
            ...
            // 向左折叠
            this.gutterLeftClick.emit({gutterNum, sizes});
        } else if (type === 'right') {
            // 向右折叠
            this.gutterRightClick.next({gutterNum, sizes});
        }
    }
